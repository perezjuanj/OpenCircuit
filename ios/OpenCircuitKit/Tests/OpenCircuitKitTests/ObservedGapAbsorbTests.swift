...
///
/// ⚠️ THE DEFAULT IS 0 = OFF, AND THE MEASUREMENT IS WHY. See the constant's doc comment: on the
/// 2026-08-19 corpus the guard changes exactly two nights, and the second one (R3 2026-08-12,
/// coverage 0.894) carries no recoverable ground truth, so the corpus cannot say whether the guard
/// helps it or truncates a genuinely fragmented night. It ships as an instrument, not a fix.
final class ObservedGapAbsorbTests: XCTestCase {
    ...
    /// THE DEFAULT IS OFF. Byte-identical to the pre-guard code on a fixture that the guard DOES
    /// change when enabled — so this is a live no-op test, not a vacuous one.
    func testDefaultCutIsZeroAndIsByteIdenticalToPreGuardScoping() {
        XCTAssertEqual(BulkSleep.observedGapAbsorbCoverageCut, 0,
                       "the shipped default must stay 0 until a fit justifies moving it")
        let union = juanShapedUnion()
        let shipped = BulkSleep.latestNightRecords(from: union)
        let explicitlyOff = BulkSleep.latestNightRecords(from: union, observedGapCoverageCut: 0)
        XCTAssertEqual(shipped.map(\.counter), explicitlyOff.map(\.counter))
        // Pre-guard behaviour: the evening block IS absorbed, so the slice reaches back to it.
        XCTAssertNotNil(firstKept(shipped))
        XCTAssertLessThanOrEqual(firstKept(shipped)!, at(20, 30),
                                 "master absorbs the evening block — if this stops being true the "
                                 + "kill-switch test has gone vacuous")
    }

    /// A negative cut is also OFF (the `> 0` guard), so a mis-set flag degrades to master.
    func testNegativeCutIsAlsoOff() {
        let union = juanShapedUnion()
        XCTAssertEqual(BulkSleep.latestNightRecords(from: union, observedGapCoverageCut: -1).map(\.counter),
                       BulkSleep.latestNightRecords(from: union).map(\.counter))
    }
    ...
    /// This is the known false-positive class and the reason the default is 0.
    func testEnabledCutAlsoDropsARealMidNightBout() {
        ...
        let off = BulkSleep.latestNightRecords(from: union)
        let on = BulkSleep.latestNightRecords(from: union, observedGapCoverageCut: 0.5)
