...
| `docs/RUNBOOK_SCHEMA_MIGRATION_REHEARSAL.md` | **MANDATORY before shipping any SwiftData schema change — rehearse the upgrade on a real phone from a PRE-45 build. Build 44 deleted every raw history row on upgrade; a simulator pass and a current-build store both skip the defect** |
